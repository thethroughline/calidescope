...
Seventeen pages, **2,596 words total** — an average of 153 a page. Generated from `build.py`.
...
2. **Keep your existing `privacy.html` and `terms.html`.** They're the only links pointing at files not generated by `build.py`. They still carry the **old** design — worth regenerating them from `build.py` at some point so the whole site matches.
...
17 pages load · zero JavaScript errors · zero broken internal links · one `<h1>` per page · no third-party font calls · no browser storage · no pure white · clean at 1400px and 390px.
