...
[calidescope.llc/next](https://calidescope.llc/next) is the September design handoff
("Calidescope's Promise and Proof", `Calidescope Site.dc.html`): a full-screen grid of
40 cards, 9 rows × 5 columns, one card per viewport, no page scroll. Down moves through the
story, right moves through one situation's beats. Drag, wheel, arrow keys, the d-pad, or the
menu (tap the mark, or press `M`).

It is **not** generated by `build.py`. `next/index.html` is built by **`next/build.py`** from
`next/source/Calidescope Site.dc.html` — the handoff file, verbatim — which keeps the template and
styles as designed and swaps three things:

- the design tool's runtime (`support.js`, which pulls React, ReactDOM and Babel from unpkg
  at load) for `next/app.js`, a plain-JavaScript port of its logic class, inlined;
- Google Fonts for the faces in `assets/fonts/` — no third-party call, as `privacy.html` says;
- the Formspree placeholder for `/api/contact`, with a honeypot and an in-card reply.
