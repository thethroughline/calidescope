...
## `/next` — the card-grid site, for testing

[calidescope.llc/next](https://calidescope.llc/next) is the September design: a full-screen grid
of 40 cards, 9 rows × 5 columns, one card per viewport, no page scroll. Down moves through the
story, right through one situation's beats. Drag, wheel, arrow keys, the arrows on screen, or the
menu (tap the mark, tap any empty card, or press `M`).

The arrows are the "therefore." A situation card ends on an unresolved *but*; left is *therefore,
if nothing changes*, right is *therefore, once it's fixed*. Labels name the destination, never the
gesture, and appear only where a move opens new ground — the situation card is the one fork, so it
is the one card with both. Colour rides on the arrow, never the words. The whole argument is in
`next/source/NOTES.md`.

It is **not** generated by `build.py`. `next/index.html` is built by **`next/build.py`** from
`next/source/calidescope-situations_2.html` — the designer's page, verbatim: its CSS, its markup,
its script — plus what a public URL needs:

- Google Fonts → the faces in `assets/fonts/`, so there is no third-party call, as `privacy.html` says;
- each row's cards sorted into column order in the DOM, so Tab and a screen reader walk the story
  in the order the grid shows it;
- the Formspree placeholder → `/api/contact`, with a honeypot and an in-card reply;
- `next/app.js`, appended: a typing guard (keys in the form edit text — they do not move the grid
  or open the menu) and the form handler.

Change the source or `next/app.js`, run `python3 next/build.py`. **Never hand-edit `next/index.html`.**

`noindex` on the page and `X-Robots-Tag` on the route. When it graduates, it replaces `index.html`
and the redirects follow; until then the two sites share the endpoint and the fonts, nothing else.
